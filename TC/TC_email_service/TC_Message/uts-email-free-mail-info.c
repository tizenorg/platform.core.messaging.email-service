...
#include "uts-email-free-mail-info.h"
...
/*Testcase   :		uts_Email_Get_Free_Mail_Info_01
  TestObjective  :	To Free  mail info 
  APIs Tested    :	int email_free_mail_data(email_mail_data_t** mail_list, int count)
 */

static void uts_Email_Free_Mail_Info_01()
{
	int err_code = EMAIL_ERROR_NONE;
	int count = 0;
	...
	tet_infoline("uts_Email_Free_Mail_Info_01 Begin\n");
			...
			if (EMAIL_ERROR_NONE == err_code) {
				...
				if (count  > 0) {
					...
					if (EMAIL_ERROR_NONE == err_code) {
						...
						err_code = email_free_mail_data(&mail_data, 1);
						if (EMAIL_ERROR_NONE == err_code) {
							tet_infoline("Email free mail info  :  success\n");
							tet_result(TET_PASS);
						}
						else {
							tet_printf("Email free mail info   :  Failed err_code[%d] \n", err_code);
							tet_result(TET_FAIL);
						}
...
/*Testcase   :		uts_Email_Free_Mail_Info_02
  TestObjective  :	To validate parameter for free mail info
  APIs Tested    :	int email_free_mail_data(email_mail_data_t** mail_list, int count)
 */

static void uts_Email_Free_Mail_Info_02()
{
	int err_code = EMAIL_ERROR_NONE;
	email_mail_data_t *mail_data = NULL;

	tet_infoline("uts_Email_Free_Mail_Info_02 Begin\n");

	err_code = email_free_mail_data(NULL, 1);
	...
	if (EMAIL_ERROR_NONE != err_code) {
		...
		tet_printf("Email free mail info   :  Failed \n");
		tet_result(TET_FAIL);
	}
