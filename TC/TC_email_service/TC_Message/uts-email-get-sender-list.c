...
#include "uts-email-get-sender-list.h"
...
/*Testcase   :		uts_Email_Get_Sender_List_01
  TestObjective  :	To get the sender list
  APIs Tested    :	int email_get_sender_list(int account_id, const char *mailbox_name, int search_type, char *search_value, emf_sort_type_t sorting, emf_sender_list_t** sender_list, int *sender_count)
 */

static void uts_Email_Get_Sender_List_01()
{
	...
	tet_infoline("uts_Email_Get_Sender_List_01 Begin\n");
...
/*Testcase   :		uts_Email_Get_Sender_List_02
  TestObjective  :	To validate parameter for account_id in get the sender list
  APIs Tested    :	int email_get_sender_list(int account_id, const char *mailbox_name, int search_type, char *search_value, emf_sort_type_t sorting, emf_sender_list_t** sender_list, int *sender_count)
 */

static void uts_Email_Get_Sender_List_02()
{
	char *MailBox_name = NULL;
	int err_code = EMF_ERROR_NONE;
	int sorting = 0;
	int account_id = -1;	/*  check thi */
	int sender_count = 0;
	emf_sender_list_t *sender_list = NULL;

	tet_infoline("uts_Email_Get_Sender_List_02 Begin\n");
	...
	if (EMF_ERROR_NONE != err_code) {
		...
		tet_printf("Email get sender list failed\n");
		tet_result(TET_FAIL);
	}
...
/*Testcase   :		uts_Email_Get_Sender_List_03
  TestObjective  :	To validate parameter for sender_list in get the sender list
  APIs Tested    :	int email_get_sender_list(int account_id, const char *mailbox_name, int search_type, char *search_value, emf_sort_type_t sorting, emf_sender_list_t** sender_list, int *sender_count)
 */

static void uts_Email_Get_Sender_List_03()
{
	char *MailBox_name = NULL;
	int err_code = EMF_ERROR_NONE;
	int sorting = 0;
	...
	tet_infoline("uts_Email_Get_Sender_List_03 Begin\n");

	sorting = EMF_SORT_DATETIME_HIGH;
	err_code = email_get_sender_list(account_id , MailBox_name , EMF_SEARCH_FILTER_NONE, NULL, sorting , NULL , &sender_count);
	if (EMF_ERROR_NONE != err_code) {
		tet_infoline("Email get sender list Success\n");
		tet_result(TET_PASS);
	}
...
/*Testcase   :		uts_Email_Get_Sender_List_04
  TestObjective  :	To validate parameter for sender_count in  get the sender list
  APIs Tested    :	int email_get_sender_list(int account_id, const char *mailbox_name, int search_type, char *search_value, emf_sort_type_t sorting, emf_sender_list_t** sender_list, int *sender_count)
 */

static void uts_Email_Get_Sender_List_04()
{
	char *MailBox_name = NULL;
	int err_code = EMF_ERROR_NONE;
	int sorting = 0;
	...
	tet_infoline("uts_Email_Get_Sender_List_04 Begin\n");

	sorting = EMF_SORT_DATETIME_HIGH;
	err_code = email_get_sender_list(account_id , MailBox_name , EMF_SEARCH_FILTER_NONE, NULL, sorting , &sender_list , 0);
	if (EMF_ERROR_NONE != err_code) {
		tet_infoline("Email get sender list Success\n");
		tet_result(TET_PASS);
	}
