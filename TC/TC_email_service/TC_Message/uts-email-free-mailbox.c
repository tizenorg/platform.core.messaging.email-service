...
#include "uts-email-free-mailbox.h"
...
/*Testcase   :		uts_Email_Free_Mailbox_01
  TestObjective  :	Free allocated memory for mailbox information.  
  APIs Tested    :	int email_free_mailbox(email_mailbox_t **mailbox_list, int count)  
 */

static void uts_Email_Free_Mailbox_01()
{
	int err_code = EMAIL_ERROR_NONE;
	int count = 0;
	...
	tet_infoline("utc_Email_Free_Mailbox_01 Begin\n");
	...
	err_code = email_get_mailbox_list(account[0].account_id, 1, &mbox, &count);
	if (EMAIL_ERROR_NONE != err_code || count < 0) {
		tet_printf("Email Get Mailbox  List Failed  : err_code[%d]", err_code);
		tet_result(TET_UNRESOLVED);
		return;
	
	}
	err_code = email_free_mailbox(&mbox, count);
	if (EMAIL_ERROR_NONE == err_code) {
		tet_infoline("Email freeMailbox   :  success\n");
		tet_result(TET_PASS);
	}
	else {
		tet_printf("Email freet Mailbox :  Failed err_code[%d] \n", err_code);
		tet_result(TET_FAIL);
	}
}
...
/*Testcase   :		uts_Email_Free_Mailbox_02
  TestObjective  :	validate parameter  Free allocated memory for mailbox information.  
  APIs Tested    :	int email_free_mailbox(email_mailbox_t **mailbox_list, int count)  
 */

static void uts_Email_Free_Mailbox_02()
{
	int err_code = EMAIL_ERROR_NONE;
	int count = 1;

	tet_infoline("utc_Email_Free_Mailbox_02 Begin\n");

	err_code = email_free_mailbox(NULL, count);
	if (EMAIL_ERROR_NONE != err_code) {
		tet_infoline("Email Free Mailbox  :  success\n");
		tet_result(TET_PASS);
	}
	else {
		tet_printf("Email Free Mailbox   :  Failed  \n");
		tet_result(TET_FAIL);
	}
}
