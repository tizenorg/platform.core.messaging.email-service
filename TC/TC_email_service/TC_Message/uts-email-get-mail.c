...
/*Testcase   :		uts_Email_Get_Mail_01
  TestObjective  :	To Get  mail  
  APIs Tested    :	int email_get_mail(emf_mailbox_t *mailbox,  int mail_id, emf_mail_t **mail)  
 */

static void uts_Email_Get_Mail_01()
{
	int err_code = EMF_ERROR_NONE;
	int count = 0;
	emf_mailbox_t mbox;
	...
	tet_infoline("utc_EmailGet_Mail_01 Begin\n");
	...
	err_code = email_get_mail(&mbox, mail_list[0].mail_id , &pMail);
	if (EMF_ERROR_NONE == err_code) {
		tet_infoline("Email get  mail  :  success\n");
		tet_result(TET_PASS);
	}
	else {
		tet_printf("Email get mail  :  Failed err_code[%d] \n", err_code);
		tet_result(TET_FAIL);
	}
}


/*Testcase   :		uts_Email_Get_Mail_02
  TestObjective  :	To validate parameter mail_id  for Get mail
  APIs Tested    :	int email_get_mail(emf_mailbox_t *mailbox,  int mail_id, emf_mail_t **mail)  
 */

static void uts_Email_Get_Mail_02()
{
	int err_code = EMF_ERROR_NONE;
	emf_mail_t *pMail = NULL;
	emf_mailbox_t mbox;

	tet_infoline("utc_Email_Get_Mail_02 Begin\n");

	memset(&mbox, 0x00, sizeof(emf_mailbox_t));
	mbox.name = strdup("INBOX");

	err_code = email_get_mail(&mbox, 0 , &pMail);
	if (EMF_ERROR_NONE != err_code) {
		tet_infoline("Email Get mail  :  success\n");
		tet_result(TET_PASS);
	}
	else {
		tet_printf("Email Get mail  :  Failed \n");
		tet_result(TET_FAIL);
	}
}
