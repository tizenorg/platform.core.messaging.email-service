...
#include "uts-email-get-attachment-info.h"
...
/*Testcase   :		uts_Email_Get_Attachment_Info_01
  TestObjective  :	Get a mail attachment
  APIs Tested    :	int email_get_attachment_data_list(int input_mail_id, email_attachment_data_t **output_attachment_data, int *output_attachment_count)
 */

static void uts_Email_Get_Attachment_Info_01()
{
	int err_code = EMAIL_ERROR_NONE;
	int i = 0;
	...
	tet_infoline("uts_Email_Get_Attachment_Info_01 Begin\n");
	...
	for (i ; i < count ; i++) {
		if(mail_list[i].has_attachment) {
			...
			if (EMAIL_ERROR_NONE == err_code) {
				tet_infoline("Email get attachment info  :  success\n");
				tet_result(TET_PASS);
				return;
			}else {
				tet_printf("Email get attachment info  :  Failed err_code[%d]\n", err_code);
				tet_result(TET_FAIL);
				return;
			}
		}
...
/*Testcase   :		uts_Email_Get_Attachment_Info_02
  TestObjective  :	Validate Parameter for mailbox in  Get a mail attachment
  APIs Tested    :	int email_get_attachment_data_list(int input_mail_id, email_attachment_data_t **output_attachment_data, int *output_attachment_count)
 */

static void uts_Email_Get_Attachment_Info_02()
{
	int err_code = EMAIL_ERROR_NONE;
	int mail_id = 1;
	email_attachment_data_t *mail_attach_info = NULL;
	...
	tet_infoline("uts_Email_Get_Attachment_Info_02 Begin\n");

	err_code = email_get_attachment_data_list(0, &mail_attach_info, &count);
	if (EMAIL_ERROR_NONE != err_code) {
		tet_infoline("Email get attachment info  :  success\n");
		tet_result(TET_PASS);
		return;
		...
		tet_printf("Email get attachment info  :  Failed \n");
		tet_result(TET_FAIL);
		return;
	}
...
/*Testcase   :		uts_Email_Get_Attachment_Info_03
  TestObjective  :	Validate Parameter for mailbox account_id in  Get a mail attachment
  APIs Tested    :	int email_get_attachment_data_list(int input_mail_id, email_attachment_data_t **output_attachment_data, int *output_attachment_count)
 */

static void uts_Email_Get_Attachment_Info_03()
{
	int err_code = EMAIL_ERROR_NONE;
	int mail_id = 1;
	...
	tet_infoline("uts_Email_Get_Attachment_Info_03 Begin\n");

	err_code = email_get_attachment_data_list(1, NULL, &count);
	if (EMAIL_ERROR_NONE != err_code) {
		tet_infoline("Email get attachment info  :  success\n");
		tet_result(TET_PASS);
		return;
