...
/*Testcase   :		uts_Email_Get_Free_Mail_01
  TestObjective  :	To Free  mail  
  APIs Tested    :	int email_free_mail(emf_mail_t **mail_list, int count)  
 */

static void uts_Email_Free_Mail_01()
{
	int err_code = EMF_ERROR_NONE;
	int i = 0;
	int count = 0;
	...
	tet_infoline("utc_Email_Free_Mail_Info_01 Begin\n");
	...
	err_code = email_free_mail(&pMail , 1);
	if (EMF_ERROR_NONE == err_code) {
		tet_infoline("Email free mail  :  success\n");
		tet_result(TET_PASS);
	}
	else {
		tet_printf("Email free mail  :  Failed err_code[%d] \n", err_code);
		tet_result(TET_FAIL);
	}
}


/*Testcase   :		uts_Email_Free_Mail_02
  TestObjective  :	To validate parameter for free mail
  APIs Tested    :	int email_free_mail(emf_mail_t **mail_list, int count)  
 */

static void uts_Email_Free_Mail_02()
{
	int err_code = EMF_ERROR_NONE;

	tet_infoline("utc_Email_Free_Mail_02 Begin\n");

	err_code = email_free_mail(NULL , 1);
	if (EMF_ERROR_NONE != err_code) {
		tet_infoline("Email free mail  :  success\n");
		tet_result(TET_PASS);
	}
	else {
		tet_printf("Email free mail  :  Failed \n");
		tet_result(TET_FAIL);
	}
}
