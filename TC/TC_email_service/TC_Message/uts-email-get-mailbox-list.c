...
/*Testcase   :		uts_Email_Get_Mailbox_List_01
  TestObjective  :	To Get all mailboxes from account.  
  APIs Tested    :	int email_get_mailbox_list(int account_id, email_mailbox_t **mailbox_list, int *count)  
 */
...
static void uts_Email_Get_Mailbox_List_01()
{
	...
	int mailbox_sync_type = 1;
	email_account_t *account = NULL ;


	tet_infoline("utc_Email_Get_Mailbox_List_01 Begin\n");
	...
	err_code = email_get_mailbox_list(account[0].account_id, mailbox_sync_type, &mbox, &count);
	if (EMAIL_ERROR_NONE == err_code) {
		tet_infoline("Email Get Mailbox List  :  success\n");
		tet_result(TET_PASS);
	}
	else {
		tet_printf("Email Get Mailbox List  :  Failed err_code[%d] \n", err_code);
		tet_result(TET_FAIL);
	}
}
...
/*Testcase   :		uts_Email_Get_Mailbox_List_02
  TestObjective  :	To validate parameter for account_id in  Get all mailboxes from account.  
  APIs Tested    :	int email_get_mailbox_list(int account_id, email_mailbox_t **mailbox_list, int *count)  
 */
...
static void uts_Email_Get_Mailbox_List_02()
{
	...
	tet_infoline("utc_Email_Get_Mailbox_List_02 Begin\n");

	err_code = email_get_mailbox_list(account_id, 1, &mbox, &count);
	if (EMAIL_ERROR_NONE != err_code) {
		tet_infoline("Email Get Mailbox List  :  success\n");
		tet_result(TET_PASS);
	}
	else {
		tet_printf("Email Get Mailbox List  :  Failed  \n");
		tet_result(TET_FAIL);
	}
}
...
/*Testcase   :		uts_Email_Get_Mailbox_List_03
  TestObjective  :	To validate parameter for mailbox_list in  Get all mailboxes from account.  
  APIs Tested    :	int email_get_mailbox_list(int account_id, email_mailbox_t **mailbox_list, int *count)  
 */
...
static void uts_Email_Get_Mailbox_List_03()
{
	...
	tet_infoline("utc_Email_Get_Mailbox_List_03 Begin\n");
	...
	err_code = email_get_mailbox_list(account[0].account_id, 1, NULL, &count);
	if (EMAIL_ERROR_NONE != err_code) {
		tet_infoline("Email Get Mailbox List  :  success\n");
		tet_result(TET_PASS);
	}
...
/*Testcase   :		uts_Email_Get_Mailbox_List_04
  TestObjective  :	To validate parameter for Get all mailboxes from account.  
  APIs Tested    :	int email_get_mailbox_list(int account_id, email_mailbox_t **mailbox_list, int *count)  
 */
...
static void uts_Email_Get_Mailbox_List_04()
{
	...
	tet_infoline("utc_Email_Get_Mailbox_List_04 Begin\n");
	...
	err_code = email_get_mailbox_list(account[0].account_id, 1, &mbox, 0);
	if (EMAIL_ERROR_NONE != err_code) {
		tet_infoline("Email Get Mailbox List  :  success\n");
		tet_result(TET_PASS);
	}
