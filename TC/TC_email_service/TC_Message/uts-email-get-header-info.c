...
#include "uts-email-get-header-info.h"
...
/*Testcase   :		uts_Email_Get_Header_Info_01
  TestObjective  :	To get  mail header info 
  APIs Tested    :	int email_get_header_info(emf_mailbox_t *mailbox, int mail_id, emf_mail_head_t **head)  
 */

static void uts_Email_Get_Header_Info_01()
{
	int err_code = EMF_ERROR_NONE;
	int i = 0;
	int count = 0;
		...
		if (count > 0) {
			...
			if (EMF_ERROR_NONE == err_code) {
				...
				if (count  > 0) {
					...
					if (EMF_ERROR_NONE == err_code) {
						tet_infoline("Email get mail head info  :  success\n");
						tet_result(TET_PASS);
					}
					else {
						tet_printf("Email get mail head  info   :  Failed err_code[%d] \n", err_code);
						tet_result(TET_FAIL);
					}
...
/*Testcase   :		uts_Email_Get_Header_Info_02
  TestObjective  :	To validate parameter for mailbox in  get  mail Header info
  APIs Tested    :	int email_get_header_info(emf_mailbox_t *mailbox, int mail_id, emf_mail_head_t **head)  
 */

static void uts_Email_Get_Header_Info_02()
{
	int err_code = EMF_ERROR_NONE;
	int mail_id = 1;
	emf_mail_head_t *pMail_head = NULL;

	tet_infoline("utc_Email_Get_Header_Info_02 Begin\n");

	err_code = email_get_header_info(NULL, mail_id , &pMail_head);
	if (EMF_ERROR_NONE != err_code) {
		tet_infoline("Email get mail header info  :  success\n");
		tet_result(TET_PASS);
	}
	else {
		tet_printf("Email get mail header info   :  Failed \n");
		tet_result(TET_FAIL);
	}
}

				
/*Testcase   :		uts_Email_Get_Header_Info_03
  TestObjective  :	To validate parameter for mail_id in  get  mail header info
  APIs Tested    :	int email_get_header_info(emf_mailbox_t *mailbox, int mail_id, emf_mail_head_t **head)  
 */

static void uts_Email_Get_Header_Info_03()
{
	int err_code = EMF_ERROR_NONE;
	int mail_id = 0;
	emf_mailbox_t mbox;
	...
	tet_infoline("utc_Email_Get_Header_Info_03 Begin\n");

	memset(&mbox, 0x00, sizeof(emf_mailbox_t));
	mbox.name = strdup("INBOX");
	mbox.account_id = g_accountId ;

	err_code = email_get_header_info(&mbox, mail_id , &pMail_head);
	if (EMF_ERROR_NONE != err_code) {
		tet_infoline("Email get mail header info  :  success\n");
		tet_result(TET_PASS);
	}
...
/*Testcase   :		uts_Email_Get_Header_Info_04
  TestObjective  :	To validate parameter for info in  get  mail header info
  APIs Tested    :	int email_get_header_info(emf_mailbox_t *mailbox, int mail_id, emf_mail_head_t **head)  
 */

static void uts_Email_Get_Header_Info_04()
{
	int err_code = EMF_ERROR_NONE;
	int mail_id = 1;
	emf_mailbox_t mbox;

	tet_infoline("uts_Email_Get_Header_Info_04 Begin\n");

	memset(&mbox, 0x00, sizeof(emf_mailbox_t));
	mbox.name = strdup("INBOX");
	mbox.account_id = g_accountId ;

	err_code = email_get_info(&mbox, mail_id , NULL);
	if (EMF_ERROR_NONE != err_code) {
		tet_infoline("Email get mail Header info  :  success\n");
		tet_result(TET_PASS);
	}
