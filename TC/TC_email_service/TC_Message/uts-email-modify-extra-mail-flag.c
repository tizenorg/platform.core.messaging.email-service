...
#include "uts-email-modify-extra-mail-flag.h"
...
/*Testcase   : 		uts_Email_Modify_Extra_Mail_Flag_01
  TestObjective  : 	To modify mail extra flags
  APIs Tested    : 	int email_modify_extra_mail_flag(int mail_id, emf_extra_flag_t new_flag)
 */

static void uts_Email_Modify_Extra_Mail_Flag_01()
{
	int err_code = EMF_ERROR_NONE;
	emf_extra_flag_t new_flag ;
	int mail_id = 0;
	int flag = 0xff;
	int count = 0;
	emf_mailbox_t mbox;
	emf_mail_list_item_t *mail_list = NULL;
	memcpy(&new_flag, &flag, sizeof(emf_extra_flag_t)); 

	tet_infoline("uts_Email_Modify_Extra_Mail_Flag_01 Begin\n");
	...
	err_code = email_modify_extra_mail_flag(mail_id, new_flag);
		if (EMF_ERROR_NONE == err_code) {
		tet_printf("Email Modify Extra Flag success\n");
		tet_result(TET_PASS);
	}
	else		{
		
		tet_printf("Email Modify Extra Flag failed : error_code[%d]\n", err_code);
		tet_result(TET_FAIL);
	}	
...
/*Testcase   : 		uts_Email_Modify_Extra_Mail_Flag_02
  TestObjective  : 	To validate parameter for modify mail extra flag
  APIs Tested    : 	int email_modify_extra_mail_flag(int mail_id, emf_extra_flag_t new_flag)
 */ 
static void uts_Email_Modify_Extra_Mail_Flag_02()
{
	int err_code = EMF_ERROR_NONE;
	int mail_id = 0; /* Invalid mail id*/
	emf_extra_flag_t new_flag ; 
	int flag = 0xff;
	memcpy(&new_flag, &flag, sizeof(emf_extra_flag_t)); 


	tet_infoline("uts_Email_Modify_Extra_Mail_Flag_02 Begin\n");
	...
	tet_infoline("uts_Email_Modify_Extra_Mail_Flag_02 End\n");	
}
