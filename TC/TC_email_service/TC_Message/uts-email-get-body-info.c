...
#include "uts-email-get-body-info.h"
	...
	if (EMF_ERROR_NONE == email_service_begin()) {
		...
		if (EMF_ERROR_NONE == email_open_db()) {
			...
			if (EMF_ERROR_NONE == err_code) {
				...
				if (i >= count) {
					/*  No mail which has a body -> Add new messag */
					tet_printf("Add new email 1 \n");
...
/*Testcase   :		uts_Email_Get_Body_Info_01
  TestObjective  :	To get  mail body info 
  APIs Tested    :	int email_get_body_info(emf_mailbox_t *mailbox, int mail_id, emf_mail_body_t **body)  
 */

static void uts_Email_Get_Body_Info_01()
{
	int err_code = EMF_ERROR_NONE;
	int i = 0;
	int count = 0;
	...
	tet_infoline("utc_Email_Get_Body_Info_01 Begin\n");
...
/*Testcase   :		uts_Email_Get_Body_Info_02
  TestObjective  :	To validate parameter for mailbox in  get  mail info
  APIs Tested    :	int email_get_body_info(emf_mailbox_t *mailbox, int mail_id, emf_mail_body_t **body)  
 */

static void uts_Email_Get_Body_Info_02()
{
	int err_code = EMF_ERROR_NONE;
	int mail_id = 1;
	emf_mail_body_t *pMail_body = NULL;

	tet_infoline("utc_Email_Get_Body_Info_02 Begin\n");

	err_code = email_get_body_info(NULL, mail_id , &pMail_body);
	if (EMF_ERROR_NONE != err_code) {
		tet_infoline("Email get mail body info  :  success\n");
		tet_result(TET_PASS);
	}
	else {
		tet_printf("Email get mail body info   :  Failed \n");
		tet_result(TET_FAIL);
	}
}

				
/*Testcase   :		uts_Email_Get_Body_Info_03
  TestObjective  :	To validate parameter for mail_id in  get  mail body info
  APIs Tested    :	int email_get_body_info(emf_mailbox_t *mailbox, int mail_id, emf_mail_body_t **body)  
 */

static void uts_Email_Get_Body_Info_03()
{
	int err_code = EMF_ERROR_NONE;
	int mail_id = 0;
	emf_mailbox_t mbox;
	...
	tet_infoline("utc_Email_Get_Body_Info_03 Begin\n");

	memset(&mbox, 0x00, sizeof(emf_mailbox_t));
	mbox.name = strdup("INBOX");
	mbox.account_id = g_accountId ;

	err_code = email_get_body_info(&mbox, mail_id , &pMail_body);
	if (EMF_ERROR_NONE != err_code) {
		tet_infoline("Email get mail body  info  :  success\n");
		tet_result(TET_PASS);
	}
...
/*Testcase   :		uts_Email_Get_Body_Info_04
  TestObjective  :	To validate parameter for body in  get  mail body info
  APIs Tested    :	int email_get_body_info(emf_mailbox_t *mailbox, int mail_id, emf_mail_body_t **body)  
 */

static void uts_Email_Get_Body_Info_04()
{
	int err_code = EMF_ERROR_NONE;
	int mail_id = 1;
	emf_mailbox_t mbox;

	tet_infoline("uts_Email_Get_Body_Info_04 Begin\n");

	memset(&mbox, 0x00, sizeof(emf_mailbox_t));
	mbox.name = strdup("INBOX");
	mbox.account_id = g_accountId ;

	err_code = email_get_body_info(&mbox, mail_id , NULL);
	if (EMF_ERROR_NONE != err_code) {
		tet_infoline("Email get mail body info  :  success\n");
		tet_result(TET_PASS);
	}
