...
#include "uts-email-validate-account.h"
...
/*Testcase   : 		uts_Email_Validate_Account_01
  TestObjective  : 	To validate an email account  
  APIs Tested    : 	int email_validate_account(int account_id, unsigned *handle)
 */ 

static void uts_Email_Validate_Account_01()
{
	int err_code = EMAIL_ERROR_NONE;
	int handle;
	email_account_t *account = NULL;
	
	tet_infoline("uts_Email_Validate_Account_01 : Begin\n");
	
 	account = (email_account_t  *)malloc(sizeof(email_account_t));
	if (account) {
		memset(account , 0x00, sizeof(email_account_t));

		account->account_name		= strdup("Gmail1 POP");
		account->user_display_name	= strdup("samsung");
		...
		if (EMAIL_ERROR_NONE == err_code) {
			tet_printf("Email add account success : accountID[%d]\n", account->account_id);
			err_code = email_validate_account(account->account_id, &handle);
			if (EMAIL_ERROR_NONE == err_code) {
				tet_printf("Email validate account success\n");
				tet_result(TET_PASS);
			}
			else		{
				tet_printf("Email validate account failed : error_code[%d]\n", err_code);
				tet_result(TET_FAIL);
			}			
		}
		else {

			tet_printf("Email add account failed : err_code[%d]n", err_code);
...
/*Testcase   : 		uts_Email_Validate_Account_02
  TestObjective  : 	To validate paramete for account_id in  validate an email account  
  APIs Tested    : 	int email_validate_account(int account_id, unsigned *handle)
 */ 

static void uts_Email_Validate_Account_02()
{
	int err_code = EMAIL_ERROR_NONE;
	int handle;
	int account_id = -1;
	
	tet_infoline("uts_Email_Validate_Account_05 : Begin\n");
	
	err_code = email_validate_account(account_id, &handle);
	if (EMAIL_ERROR_NONE != err_code) {
		tet_printf("Email validate account success\n");
		tet_result(TET_PASS);
	}
	else		{
		tet_printf("Email validate account failed : \n");
		tet_result(TET_FAIL);
	}		
}
