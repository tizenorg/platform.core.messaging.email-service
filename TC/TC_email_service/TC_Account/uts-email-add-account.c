...
#include "uts-email-add-account.h"
...
/*Testcase   : 		uts_Email_Add_Account_01
  TestObjective  : 	To add  an email account
  APIs Tested    : 	int email_add_account(email_account_t *account)
 */

static void uts_Email_Add_Account_01()
{
	email_account_t *account = NULL;
	int err_code = EMAIL_ERROR_NONE;

	tet_infoline("uts_Email_Add_Account_01 Begin\n");
	...
	if (account) {
		...
		if (EMAIL_ERROR_NONE == err_code) {
			...
			tet_result(TET_PASS);
			g_accountId = account->account_id;
		}
		else		{

			tet_printf("Email add account failed : error_code[%d]\n", err_code);
			tet_result(TET_FAIL);
		}
...
/*Testcase   : 		uts_Email_Add_Account_02
  TestObjective  : 	To validate parameter for add an email account
  APIs Tested    : 	int email_add_account(email_account_t *account)
 */
static void uts_Email_Add_Account_02()
{
	int err_code = EMAIL_ERROR_NONE;
	email_account_t *pAccount = NULL ;

	tet_infoline("uts_Email_Add_Account_02 : Begin\n");

	err_code = email_add_account(pAccount);
	if (EMAIL_ERROR_NONE != err_code) {
		tet_printf("Email add account success\n");
		tet_result(TET_PASS);
	}
	else		{
		tet_printf("Email add account failed\n");
		tet_result(TET_FAIL);
	}
}
