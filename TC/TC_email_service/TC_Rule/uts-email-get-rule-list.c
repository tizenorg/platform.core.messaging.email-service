...
#include "uts-email-get-rule-list.h"
...
/*Testcase   :		uts_Email_Get_Rule_List_01
  TestObjective  :	To get all filter  rule information
  APIs Tested    :	int email_get_rule_list(email_rule_t **filtering_set, int *count) 
 */

static void uts_Email_Get_Rule_List_01()
{
	int err_code = EMAIL_ERROR_NONE ;
	int count = 0;
	...
	tet_infoline("uts_Email_Get_Rule_List_01 Begin\n");
...
/*Testcase   :		uts_Email_Get_Rule_List_02
  TestObjective  :	To validate parameter for filtering_set in get all filter rule information
  APIs Tested    :	int email_get_rule_list(email_rule_t **filtering_set, int *count) 
 */

static void uts_Email_Get_Rule_List_02()
{
	int err_code = EMAIL_ERROR_NONE ;
	int count = 0;
	email_rule_t *pRule = NULL;
	
	tet_infoline("uts_Email_Get_Rule_List_02 Begin\n");

	err_code = email_get_rule_list(NULL, &count);
	if (EMAIL_ERROR_NONE != err_code) {
		tet_infoline("Email get all filter rule Success\n");
		tet_result(TET_PASS);
	}
	else {
		tet_printf("Email get all filter rule failed\n");
		tet_result(TET_FAIL);
		}
}


/*Testcase   :		uts_Email_Get_Rule_List_03
  TestObjective  :	To validate parameter for count in get all filter rule information
  APIs Tested    :	int email_get_rule_list(email_rule_t **filtering_set, int *count) 
 */

static void uts_Email_Get_Rule_List_03()
{
	int err_code = EMAIL_ERROR_NONE ;
	int count = 0;
	email_rule_t *pRule = NULL;
	
	tet_infoline("uts_Email_Get_Rule_List_03 Begin\n");

	err_code = email_get_rule_list(&pRule, 0);
	if (EMAIL_ERROR_NONE != err_code) {
		tet_infoline("Email get all filter rule Success\n");
		tet_result(TET_PASS);
	}
