...
/*Testcase   :		uts_Email_Get_Rule_01
  TestObjective  :	To get filter information
  APIs Tested    :	int email_get_rule(int filter_id, email_rule_t **filter_info) 
 */

static void uts_Email_Get_Rule_01()
{
	int err_code = EMAIL_ERROR_NONE ;
	int filter_id = 1;
	...
	tet_infoline("uts_Email_Get_Rule_01 Begin\n");
	...
	err_code = email_get_rule(filter_id, &pRule);
	if (EMAIL_ERROR_NONE == err_code) {
		tet_infoline("Email get rule Success\n");
		tet_result(TET_PASS);
	}
	else {
		tet_printf("Email get rule failed : error_code[%d]\n", err_code);
		tet_result(TET_FAIL);
	}
	if (mailbox) {
...
/*Testcase   :		uts_Email_Get_Rule_02
  TestObjective  :	To validate parameter for filter_info in find filter rule to an email account
  APIs Tested    :	int email_get_rule(int filter_id, email_rule_t **filter_info) 
 */

static void uts_Email_Get_Rule_02()
{
	int err_code = EMAIL_ERROR_NONE ;
	email_rule_t *pRule = NULL;

	tet_infoline("uts_Email_Get_Rule_02 Begin\n");

	err_code = email_get_rule(1, NULL);
	if (EMAIL_ERROR_NONE != err_code) {
		tet_infoline("Email get rule Success\n");
		tet_result(TET_PASS);
	}
	else {
		tet_printf("Email get rule failed\n");
		tet_result(TET_FAIL);
		}
}


/*Testcase   :		uts_Email_Get_Rule_03
  TestObjective  :	To validate parameter for filter_id in find filter rule to an email account
  APIs Tested    :	int email_get_rule(int filter_id, email_rule_t **filter_info) 
 */

static void uts_Email_Get_Rule_03()
{
	int err_code = EMAIL_ERROR_NONE ;
	email_rule_t *pRule = NULL;

	tet_infoline("uts_Email_Get_Rule_03 Begin\n");

	err_code = email_get_rule(0, &pRule);
	if (EMAIL_ERROR_NONE != err_code) {
		tet_infoline("Email get rule Success\n");
		tet_result(TET_PASS);
	}
