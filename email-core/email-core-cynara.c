...
INTERNAL_FUNC void emcore_init_cynara()
{
	...
	if (ret != CYNARA_API_SUCCESS) {
		EM_DEBUG_EXCEPTION("cynara_initialize failed : [%d], error : [%d][%s]", 
							ret,
...
INTERNAL_FUNC void emcore_finish_cynara()
{
	...
	if (ret != CYNARA_API_SUCCESS) {
		EM_DEBUG_EXCEPTION("cynara_finish failed : [%d], error : [%d][%s]", 
							ret, 
							cynara_strerror(ret, errno_buf, ERRNO_BUF_SIZE),
							errno_buf);
	}
...
INTERNAL_FUNC int emcore_check_privilege(int socket_fd, int client_pid)
{
	...
	if (ret != CYNARA_API_SUCCESS) {
		EM_DEBUG_EXCEPTION("cynara_creds_socket_get_client failed : [%d], error : [%d][%s]",
							ret,
							cynara_strerror(ret, errno_buf, ERRNO_BUF_SIZE),
							errno_buf);
		goto FINISH_OFF;
	...
	if (ret != CYNARA_API_SUCCESS) {
		EM_DEBUG_EXCEPTION("cynara_creds_socket_get_user failed : [%d], error : [%d][%s]", 
							ret, 
							cynara_strerror(ret, errno_buf, ERRNO_BUF_SIZE),
							errno_buf);
		goto FINISH_OFF;
	...
	if (client_session == NULL) {
		EM_DEBUG_EXCEPTION("cynara_session_from_pid failed error : [%d][%s]", 
							cynara_strerror(ret, errno_buf, ERRNO_BUF_SIZE),
							errno_buf);
		goto FINISH_OFF;
	}
	...
	if (ret != CYNARA_API_ACCESS_ALLOWED) {
		EM_DEBUG_EXCEPTION("cynara_check failed : [%d], error : [%d][%s]", 
							ret,
							cynara_strerror(ret, errno_buf, ERRNO_BUF_SIZE),
							errno_buf);
		goto FINISH_OFF;
