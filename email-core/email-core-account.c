...
static char *emcore_get_imap_capability_string(MAILSTREAM *input_stream)
{
	...
	if ((imap_capability = imap_cap(input_stream))) {
		if (imap_capability->idle)
			EM_SAFE_STRNCAT(capability_string, "IDLE ", 512 - EM_SAFE_STRLEN(capability_string) - 1);
		if (imap_capability->quota)
			EM_SAFE_STRNCAT(capability_string, "QUOTA ", 512 - EM_SAFE_STRLEN(capability_string) - 1);
		if (imap_capability->starttls)
			EM_SAFE_STRNCAT(capability_string, "STARTTLS ", 512 - EM_SAFE_STRLEN(capability_string) - 1);
#ifdef __FEATURE_XLIST_SUPPORT__
		if (imap_capability->xlist)
			EM_SAFE_STRNCAT(capability_string, "XLIST ", 512 - EM_SAFE_STRLEN(capability_string) - 1);
